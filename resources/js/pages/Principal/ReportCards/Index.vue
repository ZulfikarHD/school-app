...
/**
 * Principal Report Cards Index - Halaman approval rapor siswa
...
import { index, show, bulkApprove } from '@/routes/principal/report-cards';
...
const localFilters = ref({
    ...
    status: props.filters.status || 'PENDING_APPROVAL'
});
...
const resetFilters = () => {
    ...
    router.visit(index.url({ tahun_ajaran: localFilters.value.tahun_ajaran, status: 'PENDING_APPROVAL' }), {
        preserveState: true,
        preserveScroll: true
    });
};
...
/**
 * Check apakah ada filter yang aktif selain status default
 */
const hasActiveFilters = computed(() => {
    return localFilters.value.semester ||
        localFilters.value.class_id ||
        (localFilters.value.status && localFilters.value.status !== 'PENDING_APPROVAL');
});
...
/**
 * Handle bulk approve untuk kelas tertentu
 */
const handleBulkApprove = async (classId: number, className: string) => {
    if (!localFilters.value.semester) {
        await modal.dialog({
            type: 'warning',
            icon: 'warning',
            title: 'Pilih Semester',
            message: 'Pilih semester terlebih dahulu sebelum melakukan bulk approve.',
    ...
    const confirmed = await modal.dialog({
        type: 'warning',
        icon: 'question',
        title: 'Approve Semua Rapor',
        message: `Anda akan meng-approve dan merilis semua rapor kelas ${className} untuk semester ${localFilters.value.semester}. Setelah dirilis, rapor akan dapat diakses oleh orang tua. Lanjutkan?`,
        ...
        <div class="max-w-7xl mx-auto space-y-6 px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
                        ...
                        <div class="flex items-center gap-3 sm:gap-4">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-slate-500 dark:text-slate-400 mt-0.5">
                                    Review dan setujui rapor siswa
                                </p>
                            </div>
                        </div>
                    </div>
            ...
            >
                <div class="grid grid-cols-2 sm:grid-cols-5 gap-4">
                    ...
                    <div class="bg-white dark:bg-zinc-900 rounded-xl p-4 shadow-sm border border-amber-200 dark:border-amber-800/50">
                        <div class="flex items-center gap-3">
                    ...
                    <div class="bg-white dark:bg-zinc-900 rounded-xl p-4 shadow-sm border border-slate-200 dark:border-zinc-800">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-slate-500 dark:text-slate-400">Dirilis</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <!-- Pending by Class (Quick Actions) -->
            <Motion
                v-if="pendingByClass.length > 0"
                :initial="{ opacity: 0, y: 10 }"
                :animate="{ opacity: 1, y: 0 }"
                :transition="{ duration: 0.3, ease: 'easeOut', delay: 0.08 }"
            >
                <div class="bg-amber-50 dark:bg-amber-900/20 rounded-2xl p-4 sm:p-6 border border-amber-200 dark:border-amber-800/50">
                    <div class="flex items-center gap-3 mb-4">
                        <Clock class="w-5 h-5 text-amber-600 dark:text-amber-400" />
                        <h3 class="font-semibold text-amber-800 dark:text-amber-300">
                            Rapor Menunggu Persetujuan
                        </h3>
                    </div>
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                        <div
                            v-for="item in pendingByClass"
                            :key="item.class_id"
                            class="flex items-center justify-between bg-white dark:bg-zinc-800 rounded-xl p-3"
                        >
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <p class="font-medium text-slate-900 dark:text-slate-100">{{ item.class_name }}</p>
                                    <p class="text-xs text-amber-600 dark:text-amber-400">{{ item.pending_count }} rapor pending</p>
                                </div>
                            </div>
                            <Motion :whileTap="{ scale: 0.95 }">
                                <button
                                    v-if="localFilters.semester"
                                    type="button"
                                    @click="handleBulkApprove(item.class_id, item.class_name)"
                                    :disabled="isSubmitting"
                                    class="p-2 rounded-lg bg-emerald-500 text-white hover:bg-emerald-600 transition-colors disabled:opacity-50"
                                    title="Approve Semua"
                                >
                                    <Check class="w-4 h-4" />
                                </button>
                    ...
                    <p v-if="!localFilters.semester" class="text-xs text-amber-600 dark:text-amber-400 mt-3">
                        * Pilih semester untuk melakukan bulk approve
                    </p>
                </div>
            </Motion>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
                            <div>
                                ...
                                <select
                                    ...
                                    class="w-full px-3 py-2.5 rounded-lg border border-slate-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-slate-900 dark:text-slate-100 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500"
                                >
                                    <option
                            ...
                            <div class="flex items-end gap-2">
                                <Motion :whileTap="{ scale: 0.97 }" class="flex-1">
                                    <button
                                        type="button"
                                        @click="applyFilters"
                                        class="w-full px-4 py-2.5 bg-amber-500 text-white rounded-lg hover:bg-amber-600 transition-colors font-medium text-sm"
                                    >
