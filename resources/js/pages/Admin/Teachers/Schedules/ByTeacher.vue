...
/**
 * ByTeacher Page - Matrix view jadwal per guru
 * Menampilkan jadwal mengajar satu guru dalam format matrix (hari x waktu)
 */
import { Head, Link, router } from '@inertiajs/vue3';
import { ArrowLeft, Calendar, User, FileDown, ChevronDown } from 'lucide-vue-next';
...
// Total jam pelajaran (45 menit per JP)
const totalJamPelajaran = computed(() => {
    let totalMinutes = 0;
    props.schedules.forEach(schedule => {
        const start = schedule.jam_mulai.split(':').map(Number);
        const end = schedule.jam_selesai.split(':').map(Number);
    ...
    return Math.round(totalMinutes / 45 * 10) / 10;
});
...
const changeAcademicYear = (yearId: string) => {
    ...
    router.get(`/admin/teachers/schedules/by-teacher/${props.teacher.id}`, {
...
<template>
    <AppLayout :title="`Jadwal - ${teacher.nama_lengkap}`">
        <Head :title="`Jadwal - ${teacher.nama_lengkap}`" />
                    ...
                    <div class="flex flex-col gap-4">
                        ...
                        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-4 border-t border-slate-200 dark:border-zinc-700">
                            ...
                            <div class="flex items-center gap-6">
                                ...
                                <div class="h-10 w-px bg-slate-200 dark:bg-zinc-700"></div>
                                <div>
                                    <p class="text-2xl font-bold text-slate-900 dark:text-slate-100">{{ totalJamPelajaran }}</p>
                                    <p class="text-sm text-slate-500 dark:text-slate-400">JP/Minggu</p>
                                </div>
                            </div>
