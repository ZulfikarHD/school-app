...
/**
 * Payment Category Create Page - Halaman pembuatan kategori pembayaran baru
...
import { index, store } from '@/routes/admin/payment-categories';
...
const form = useForm({
    ...
    due_day: 10,
    tahun_ajaran: '',
    class_prices: [] as Array<{ class_id: number | null; nominal: number }>,
});

const haptics = useHaptics();
const modal = useModal();
const showClassPrices = ref(false);
...
if (!form.tahun_ajaran) {
    form.tahun_ajaran = currentAcademicYear.value;
}
...
const submit = () => {
    ...
    form.transform((data) => ({
        ...
        onSuccess: () => {
            haptics.success();
            modal.success('Kategori pembayaran berhasil ditambahkan.');
        },
        onError: () => {
            haptics.error();
            modal.error('Gagal menambahkan kategori pembayaran. Periksa kembali input Anda.');
        },
    });
};
</script>
...
<template>
    <AppLayout title="Tambah Kategori Pembayaran">
                    ...
                    <div class="flex items-center gap-3 sm:gap-4">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">
                                Buat kategori pembayaran baru (SPP, Uang Gedung, dll.)
                            </p>
                        </div>
                    </div>
                </div>
