<script setup lang="ts">
import { Head, Link, useForm } from '@inertiajs/vue3';
import { ChevronLeft, Save } from 'lucide-vue-next';
import { Motion } from 'motion-v';
import AppLayout from '@/components/layouts/AppLayout.vue';
import StudentForm from '@/components/features/students/StudentForm.vue';
import { index as studentsIndex, update as updateStudent } from '@/routes/admin/students';
import { useHaptics } from '@/composables/useHaptics';
import { useModal } from '@/composables/useModal';
import type { Student, Guardian } from '@/types/student';

interface Props {
    student: Student & { guardians: Guardian[] };
    classes: Array<{ id: number; nama: string }>;
}

const props = defineProps<Props>();
const haptics = useHaptics();
const modal = useModal();

// Map guardians
const mapGuardians = () => {
    const ayah = props.student.guardians.find(g => g.hubungan === 'ayah');
    const ibu = props.student.guardians.find(g => g.hubungan === 'ibu');
    const wali = props.student.guardians.find(g => g.hubungan === 'wali');

    return {
        ayah: ayah ? {
            nama_lengkap: ayah.nama_lengkap,
            nik: ayah.nik,
            pekerjaan: ayah.pekerjaan,
            pendidikan: ayah.pendidikan,
            penghasilan: ayah.penghasilan,
            no_hp: ayah.no_hp || '',
            email: ayah.email || '',
            alamat: ayah.alamat || '',
            is_primary_contact: ayah.pivot?.is_primary_contact || false
        } : {
            nama_lengkap: '',
            nik: '',
            pekerjaan: '',
            pendidikan: '',
            penghasilan: '',
            no_hp: '',
            email: '',
            alamat: '',
            is_primary_contact: false
        },
        ibu: ibu ? {
            nama_lengkap: ibu.nama_lengkap,
            nik: ibu.nik,
            pekerjaan: ibu.pekerjaan,
            pendidikan: ibu.pendidikan,
            penghasilan: ibu.penghasilan,
            no_hp: ibu.no_hp || '',
            email: ibu.email || '',
            alamat: ibu.alamat || '',
            is_primary_contact: ibu.pivot?.is_primary_contact || false
        } : {
            nama_lengkap: '',
            nik: '',
            pekerjaan: '',
            pendidikan: '',
            penghasilan: '',
            no_hp: '',
            email: '',
            alamat: '',
            is_primary_contact: false
        },
        wali: wali ? {
            nama_lengkap: wali.nama_lengkap,
            nik: wali.nik,
            pekerjaan: wali.pekerjaan,
            pendidikan: wali.pendidikan,
            penghasilan: wali.penghasilan,
            no_hp: wali.no_hp || '',
            email: wali.email || '',
            alamat: wali.alamat || '',
            is_primary_contact: wali.pivot?.is_primary_contact || false
        } : {
            nama_lengkap: '',
            nik: '',
            pekerjaan: '',
            pendidikan: '',
            penghasilan: '',
            no_hp: '',
            email: '',
            alamat: '',
            is_primary_contact: false
        }
    };
};

const guardians = mapGuardians();

const form = useForm({
    nama_lengkap: props.student.nama_lengkap,
    nama_panggilan: props.student.nama_panggilan || '',
    nik: props.student.nik,
    nisn: props.student.nisn,
    jenis_kelamin: props.student.jenis_kelamin,
    tempat_lahir: props.student.tempat_lahir,
    tanggal_lahir: props.student.tanggal_lahir,
    agama: props.student.agama,
    anak_ke: props.student.anak_ke,
    jumlah_saudara: props.student.jumlah_saudara,
    status_keluarga: props.student.status_keluarga,
    foto: null as File | null,
    alamat: props.student.alamat,
    rt_rw: props.student.rt_rw || '',
    kelurahan: props.student.kelurahan,
    kecamatan: props.student.kecamatan,
    kota: props.student.kota,
    provinsi: props.student.provinsi,
    kode_pos: props.student.kode_pos || '',
    no_hp: props.student.no_hp || '',
    email: props.student.email || '',
    kelas_id: props.student.kelas_id || '',
    tahun_ajaran_masuk: props.student.tahun_ajaran_masuk,
    tanggal_masuk: props.student.tanggal_masuk,
    ayah: guardians.ayah,
    ibu: guardians.ibu,
    wali: guardians.wali
});

const submit = () => {
    haptics.medium();
    form.put(updateStudent(props.student.id).url, {
        onSuccess: () => {
            haptics.success();
            modal.success('Data siswa berhasil diperbarui.');
        },
        onError: () => {
            haptics.error();
        }
    });
};
</script>

<template>
    <AppLayout title="Edit Siswa">
        <Head :title="`Edit - ${student.nama_lengkap}`" />

        <div class="max-w-5xl mx-auto space-y-6">
            <Motion
                :initial="{ opacity: 0, y: -10 }"
                :animate="{ opacity: 1, y: 0 }"
                :transition="{ duration: 0.3, ease: 'easeOut' }"
            >
                <div class="bg-white dark:bg-zinc-900 rounded-2xl p-6 shadow-sm border border-gray-200 dark:border-zinc-800">
                    <div class="flex items-center gap-4">
                        <Motion :whileTap="{ scale: 0.97 }">
                            <Link
                                :href="studentsIndex().url"
                                @click="haptics.light()"
                                class="p-2.5 text-gray-500 hover:text-gray-700 hover:bg-gray-100 dark:hover:bg-zinc-800 rounded-xl transition-colors"
                            >
                                <ChevronLeft class="w-5 h-5" />
                            </Link>
                        </Motion>
                        <div class="flex-1">
                            <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100">Edit Data Siswa</h1>
                            <p class="text-gray-500 dark:text-gray-400 text-sm mt-0.5">
                                {{ student.nama_lengkap }} • NIS: {{ student.nis }}
                            </p>
                        </div>
                    </div>
                </div>
            </Motion>

            <form @submit.prevent="submit" class="space-y-6">
                <StudentForm
                    :form="form"
                    mode="edit"
                    :classes="classes"
                />

                <!-- Actions - Fixed bottom with safe area -->
                <Motion
                    :initial="{ opacity: 0, y: 10 }"
                    :animate="{ opacity: 1, y: 0 }"
                    :transition="{ duration: 0.3, ease: 'easeOut', delay: 0.2 }"
                >
                    <div class="sticky bottom-0 z-10 -mx-4 sm:mx-0 px-4 sm:px-0 pb-6 pt-4 md:pb-4">
                        <div class="bg-white/98 dark:bg-zinc-900/98 border border-slate-200 dark:border-zinc-800 rounded-2xl p-4 flex flex-col sm:flex-row items-center justify-end gap-3 shadow-xl backdrop-blur-sm">
                            <Motion :whileTap="{ scale: 0.97 }">
                                <Link
                                    :href="studentsIndex().url"
                                    @click="haptics.light()"
                                    class="w-full sm:w-auto px-5 py-3 min-h-[48px] text-slate-700 bg-slate-50 border border-slate-300 rounded-xl hover:bg-slate-100 dark:bg-zinc-800 dark:border-zinc-700 dark:text-slate-300 dark:hover:bg-zinc-700 transition-colors font-medium text-center flex items-center justify-center"
                                >
                                    Batal
                                </Link>
                            </Motion>
                            <Motion :whileTap="{ scale: 0.97 }">
                                <button
                                    type="submit"
                                    :disabled="form.processing"
                                    class="w-full sm:w-auto flex items-center justify-center gap-2 px-6 py-3 min-h-[48px] bg-emerald-500 text-white rounded-xl hover:bg-emerald-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors shadow-lg shadow-emerald-500/25 font-semibold"
                                >
                                    <Save class="w-5 h-5" />
                                    <span v-if="form.processing">Menyimpan...</span>
                                    <span v-else>Simpan Perubahan</span>
                                </button>
                            </Motion>
                        </div>
                    </div>
                </Motion>
            </form>
        </div>
    </AppLayout>
</template>
