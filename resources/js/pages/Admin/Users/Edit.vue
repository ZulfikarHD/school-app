...
/**
 * Users Edit Page - Halaman edit data user
...
import { ChevronLeft, Edit, Save } from 'lucide-vue-next';
...
const submit = () => {
    ...
    form.put(update(props.user.id).url, {
        onSuccess: () => {
            haptics.success();
            modal.success('Data user berhasil diperbarui.');
        },
        onError: () => {
            haptics.error();
            modal.error('Gagal memperbarui data user. Periksa kembali input Anda.');
        },
    });
};
</script>
                    ...
                    <div class="flex items-center gap-3 sm:gap-4">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-sm text-slate-500 dark:text-slate-400 mt-1 truncate">
                                Perbarui informasi <span class="font-medium text-slate-700 dark:text-slate-300">{{ user.name }}</span>
                            </p>
                        </div>
                    </div>
                </div>
